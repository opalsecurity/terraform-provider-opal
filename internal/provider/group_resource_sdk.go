...
func (r *GroupResourceModel) RefreshFromSharedGroup(ctx context.Context, resp *shared.Group) diag.Diagnostics {
	...
	if resp != nil {
		...
		if resp.RemoteInfo == nil {
			...
			if resp.RemoteInfo.TwingateGroupSynced == nil {
				r.RemoteInfo.TwingateGroupSynced = nil
			} else {
				r.RemoteInfo.TwingateGroupSynced = &tfTypes.ActiveDirectoryGroup{}
				r.RemoteInfo.TwingateGroupSynced.GroupID = types.StringValue(resp.RemoteInfo.TwingateGroupSynced.GroupID)
			}
...
func (r *GroupResourceModel) ToSharedCreateGroupInfo(ctx context.Context) (*shared.CreateGroupInfo, diag.Diagnostics) {
	...
	if r.RemoteInfo != nil {
		...
		if r.RemoteInfo.DocusignSigningGroup != nil {
			var signingGroupID string
			signingGroupID = r.RemoteInfo.DocusignSigningGroup.SigningGroupID.ValueString()

			docusignSigningGroup = &shared.DocusignSigningGroup{
