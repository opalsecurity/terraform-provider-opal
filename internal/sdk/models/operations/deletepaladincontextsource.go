// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: a2f0e38cf881

package operations

import (
	"net/http"
)

type DeletePaladinContextSourceRequest struct {
	// The ID of the context source.
	ID string `pathParam:"style=simple,explode=true,name=context_source_id"`
	// The ID of the Paladin.
	PaladinID string `pathParam:"style=simple,explode=true,name=paladin_id"`
}

func (d *DeletePaladinContextSourceRequest) GetID() string {
	if d == nil {
		return ""
	}
	return d.ID
}

func (d *DeletePaladinContextSourceRequest) GetPaladinID() string {
	if d == nil {
		return ""
	}
	return d.PaladinID
}

type DeletePaladinContextSourceResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
}

func (d *DeletePaladinContextSourceResponse) GetContentType() string {
	if d == nil {
		return ""
	}
	return d.ContentType
}

func (d *DeletePaladinContextSourceResponse) GetStatusCode() int {
	if d == nil {
		return 0
	}
	return d.StatusCode
}

func (d *DeletePaladinContextSourceResponse) GetRawResponse() *http.Response {
	if d == nil {
		return nil
	}
	return d.RawResponse
}
