...
// OpalQueries - Operations related to OpalQuery
type OpalQueries struct {
	rootSDK          *OpalAPI
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newOpalQueries(rootSDK *OpalAPI, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *OpalQueries {
	return &OpalQueries{
		rootSDK:          rootSDK,
...
func (s *OpalQueries) RunOpalQuery(ctx context.Context, request shared.RunOpalQueryRequest, opts ...operations.Option) (*operations.RunOpalQueryResponse, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/queries/run")
	if err != nil {
	...
	res := &operations.RunOpalQueryResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.OpalQueryResults
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.OpalQueryResults = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
