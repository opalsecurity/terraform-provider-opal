...
- `group_name` (String) Case-insensitive substring match against each group's `name` and `remote_name`. For example, `eng` matches `Engineering` and `platform-eng`. Not an exact or prefix-only match.
